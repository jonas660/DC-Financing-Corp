...
class LoanApplicationListView(SearchSortFilterMixin, LoginRequiredMixin, ListView):
    ...
    partial_template_name = "pages/loans/_application_table.html"

    def get_queryset(self):
        return super().get_queryset().select_related("motor", "financing_term")

    def get_filter_choices(self):
        """Provide status choices for filter dropdown."""
        return {
            'status': [('', 'All Status')] + list(LoanApplication.Status.choices),
        }
    ...
    def get_context_data(self, **kwargs: Any) -> Dict[str, Any]:
        ...
        context['sort_url_motorcycle'] = get_sort_url('motorcycle')
        context['sort_url_monthly'] = get_sort_url('monthly')
        context['sort_url_status'] = get_sort_url('status')
        context['sort_url_created'] = get_sort_url('created')
        return context
...
class LoanApplicationReturnView(LoginRequiredMixin, View):
    required_roles = ("admin", "finance")

    def post(self, request: HttpRequest, pk: int) -> HttpResponse:
        application = get_object_or_404(LoanApplication, pk=pk)
        try:
            application.return_unit()
        except ValidationError as exc:
            messages.error(request, "; ".join(exc.messages))
        else:
            messages.success(request, "Unit marked as returned. Stock has been restored.")
        return redirect("loans:detail", pk=pk)
