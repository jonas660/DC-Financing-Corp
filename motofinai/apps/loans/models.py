...
class LoanApplication(models.Model):
    ...
    def return_unit(self) -> None:
        """Mark the loan as returned and restore stock."""
        if self.status == self.Status.RETURNED:
            raise ValidationError("This application is already marked as returned.")
        # Increase stock when unit is returned (only if status was active - unit was already taken)
        if self.status == self.Status.ACTIVE and self.motor and self.motor.stock:
            try:
                self.motor.stock.increase_available(amount=1)
            except ValueError as e:
                raise ValidationError(f"Stock management error: {str(e)}")
