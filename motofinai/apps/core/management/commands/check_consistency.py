...
class Command(BaseCommand):
    ...
    def quick_check(self, verbose=False, fix=False):
        ...
        # Check for duplicate chassis/plate numbers
        from django.db.models import Count

        duplicate_plates = Motor.objects.values("chassis_plate_number").annotate(
            count=Count("id")
        ).filter(count__gt=1, chassis_plate_number__isnull=False).exclude(chassis_plate_number='')
        if duplicate_plates.exists():
            count = duplicate_plates.count()
            self.stdout.write(
                self.style.ERROR(f"  [ERROR] Found {count} duplicate chassis/plate number(s)")
            )
            total_errors += count
